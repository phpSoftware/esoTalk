...
class ETPlugin_Attachments extends ETPlugin {
	...
	public function init()
	{
		...
		function formatAttachment($attachment, $expanded = false)
		{
			...
			if (in_array($extension, array("JPG", "JPEG", "jpg", "jpeg", "png", "gif"))) {
				if ($expanded) return "<a data-fancybox='gallery' href='".$url."'><img src='".$url."' alt='".$filename."' title='Click to enlarge'></a>";
				else return "<a data-fancybox='gallery' href='".$url."' class='attachment attachment-image' target='_blank'><img src='".URL("attachment/thumb/".$attachment["attachmentId"])."' alt='".$filename."' title='Click to enlarge'></a>";
			}
