...
public static function mentionEmail($item, $member)
{
	...
	return array(
		...
		sprintf(T("email.mention.body"), name($item["fromMemberName"]), sanitizeHTML($item["data"]["title"]), $content),
		$url
	);
}
...
public static function privateAddEmail($item, $member)
{
	...
	return array(
		...
		sprintf(T("email.privateAdd.body"), sanitizeHTML($item["data"]["title"]), $content),
		$url
	);
}
...
public static function postEmail($item, $member)
{
	...
	return array(
		...
		sprintf(T("email.post.body"), name($item["fromMemberName"]), sanitizeHTML($item["data"]["title"]), $content),
		$url
	);
}
