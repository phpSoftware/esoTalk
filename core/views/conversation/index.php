...
foreach ($scrubber as $year => $months) {
	...
	if (!empty($months)) {
		...
		foreach ($months as $month) {
			...
			echo "<li class='scrubber-$index$selected' data-index='$index'><a href='".URL(makeURL("$year/$month"))."'>".$month_array[$name]."</a></li>";
		}
		echo "</ul>";
	}
...
foreach ($recentMonths as $month) {
	...
	echo "<li class='scrubber-$index$selected' data-index='$index'><a href='".URL(makeURL("$currentYear/$month"))."'>".$month_array[$name]."</a>";
	echo "</li>";
}
