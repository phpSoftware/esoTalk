...
public function links()
{
	...
	$this->content = preg_replace($pattern, "<a href='mailto:$0' class='link-email'>$0</a>", $this->content);
...
public function linksCallback($matches)
{
	...
	if (!$this->inline and C("esoTalk.format.youtube") and preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*(?:\?|&amp;)v=)|youtu\.be/)([^"&?/ ]{11})(?:(?:\?|&amp;)(.*))?%i', $matches[2], $youtube)) {
		$id = $youtube[1];
		# $options = $youtube[2];
		// vanGato - undefined offset
		if(isset($youtube[2])) $options = $youtube[2]; else $options = '';
		$width = 400;
		$height = 225;
