...
function sendEmail($to, $subject, $body, $debug=false)
{
	try {
		...
		require_once(PATH_LIBRARY.'/vendor/class.phpmailer.php');
		require_once(PATH_LIBRARY.'/vendor/class.smtp.php');
		require_once(PATH_LIBRARY.'/vendor/class.exception.php');
		// TODO - https://github.com/PHPMailer/PHPMailer/blob/master/examples/smime_signed_mail.phps
		...
		// vanGato - https://www.jonasjohn. de/snippets/php/color-inverse.htm
		...
		$mail->SetFrom( C("plugin.SMTP.username"), sanitizeForHTTP(C("esoTalk.forumTitle")) );
    $mail->AddReplyTo( C("esoTalk.emailFrom"), sanitizeForHTTP(C("esoTalk.forumTitle")) );
    $logo =PATH_UPLOADS.'/'.basename(C("esoTalk.forumLogo"));
    if (file_exists($logo)) {
		...
		$vars = array(
    	"[{SUBJECT}]"   => sanitizeForHTTP($subject),
    	"[{BODY}]"      => $body,
    	"[{URL}]"       => $url,
    	"[{CLICK}]"     => T("email.click"),
    	...
    	"[{COLOR}]"     => C("skin.".$active_skin.".primaryColor"),
    	"[{COLOR_INV}]" => color_inverse( C("skin.".$active_skin.".primaryColor") ),
    	"[{COLOR_CTA}]" => 'crimson',
		);
		if ($debug === true ) $vars["cid:logoimg"] = C('esoTalk.baseURL').C('esoTalk.forumLogo');
		$message = strtr($message, $vars);
		#$mail->Body = $body;
		$mail->Body = $message;
    if ($debug === true ) die($message);
